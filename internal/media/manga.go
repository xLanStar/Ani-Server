...
func (media *Manga) String() string {
	return fmt.Sprintf("\n  [作品] ID:%6d 類型:%s\n  - 標題:%s\n  - 簡介:%s\n  - 動漫狂ID:%6d", media.Id, MANGA, media.Title, media.Description, media.Id_cartoonmad)
}
...
func (manga *Manga) GetId_cartoonmad() uint32 {
	return manga.Id_cartoonmad
}
...
func (manga *Manga) Write(fileWriter *fastio.FileWriter) {
	...
	fileWriter.WriteUint32(manga.GetId_cartoonmad())
